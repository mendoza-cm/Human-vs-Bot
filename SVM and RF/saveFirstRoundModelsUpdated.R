...
load("./data/human_vs_dataUpdated.RData")
...
##### train models
# linear SVM
model_linear <- svm(isBot ~ rate + sd_inter_response, 
		data = myTraining_dat, 
		kernel = "linear",
		probability = TRUE,
...
# radial kernel (RBF)
model_rbf <- svm(isBot ~ rate + sd_inter_response, 
		data = myTraining_dat, 
		kernel = "radial",
		probability = TRUE,
		type = "C-classification")
...
# polynomial kernel
model_poly2 <- svm(isBot ~ rate + sd_inter_response, 
		data = myTraining_dat, 
		kernel = "polynomial", 
		degree = 2,
		probability = TRUE,
...
model_poly3 <- svm(isBot ~ rate + sd_inter_response, 
		data = myTraining_dat, 
		kernel = "polynomial", 
		degree = 3,
		probability = TRUE,
...
rf_model = randomForest(factor(isBot) ~ ., 
		data = training_data[,c("rate","sd_inter_response","duration","mean_inter_response","responses_per_session","isBot")], 
		type = c("class","prob"))

myModels$RF <- rf_model

#save(myModels, file = "./SVM and RF/svmRF_modelsUpdated.RData")
